...
let
  ...
  gtk-theme = pkgs.gruvbox-gtk-theme.override {
    iconVariants = [ "Dark" ];
...
with lib.hm.gvariant;
{
  ...
  config = lib.mkIf config.features.gnome.enable {
    features.desktopapps.enable = lib.mkDefault true;
    ...
    dconf.settings = {
      "org/gnome/shell" = {
        favorite-apps = config.features.gnome.dock-apps ++ [ "org.gnome.Settings.desktop" ];
      };
